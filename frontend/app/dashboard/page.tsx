...
function PersonalCopyCard({ file }: { file: UserFileData }) {
  ...
  // Practice forks should open in the practice tab, not the lecture tab
  const href = file.file_type === "practice"
    ? `/lecture/${file.source_id}?tab=practice`
    : `/lecture/${file.source_id}`
  ...
  return (
    <div className="min-h-screen bg-[#11111b] text-[#cdd6f4]">
      <div className="bg-[#1e1e2e] border-b border-[#313244] px-6 py-3 flex items-center justify-between">
        ...
        <UserButton afterSignOutUrl="/study/sign-in" />
      </div>
      ...
      <div className="max-w-4xl mx-auto px-6 py-10 space-y-12">
        ...
        {personalCopies.length > 0 && (
          <section>
            ...
            <p className="text-[#6c7086] text-sm mb-4">Ваши личные копии лекций</p>
        ...
        {/* Custom files created with + button */}
        {myFiles.length > 0 && (
          <MyFilesSection files={myFiles} />
        )}

        {/* Admin lecture files */}
        <section>
          <h2 className="text-lg font-bold mb-1">Лекции</h2>
          <p className="text-[#6c7086] text-sm mb-4">Выберите лекцию для изучения</p>
          {lectures.length === 0 ? (
            <p className="text-[#6c7086]">Лекции не найдены.</p>
          ) : (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        {/* Admin practice files */}
        {practices.length > 0 && (
          <section>
            <h2 className="text-lg font-bold mb-1">Практика</h2>
            <p className="text-[#6c7086] text-sm mb-4">Задания и упражнения</p>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {practices.map((practice) => (
                <Link key={practice.id} href={`/lecture/${practice.id}?tab=practice`}>
                  <div className="bg-[#1e1e2e] border border-[#313244] hover:border-[#585b70] transition-colors cursor-pointer rounded-lg p-4">
                    <div className="flex items-center justify-between">
                      <p className="text-[#cdd6f4] text-sm font-medium">{practice.title}</p>
                      {practice.is_default && (
                        <span className="text-xs bg-[#313244] text-[#a6e3a1] px-2 py-0.5 rounded">По умолчанию</span>
                      )}
                    </div>
                    <p className="text-[#6c7086] font-mono text-xs mt-1">{practice.filename}</p>
                  </div>
                </Link>
              ))}
            </div>
